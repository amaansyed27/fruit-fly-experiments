...
def run_experiment(controller_name: str, seconds: float, seed: int, device: str, demo: bool, log: bool, data_root: Path | None = None) -> dict:
    ...
    if demo:
        if sim is None:
            # random baseline has no brain; use a tiny placeholder-free mode by skipping brain panel animation.
    ...
    logger = RunLogger(Path("results/experiment_001_pong"), seed) if log else None
    dt = 0.020
    ...
    try:
        for step in range(steps):
            ...
            if decision: latencies.append(brain_latency)
            elapsed = perf_counter() - started
            fps = (step + 1) / max(elapsed, 1e-9)
            if logger:
                logger.write({
                    "timestamp": datetime.now(timezone.utc).isoformat(), "seed": seed, "step": step+1,
                    "simulated_time_s": (step+1)*dt, "action": ACTION_NAMES[action],
                    "left_output": decision.left_activity if decision else "", "right_output": decision.right_activity if decision else "",
                    "ball_x": snapshot.ball_x, "ball_y": snapshot.ball_y, "paddle_y": snapshot.fly_paddle_y,
                    "opponent_y": snapshot.opponent_paddle_y, "fly_score": snapshot.fly_score, "opponent_score": snapshot.opponent_score,
                    "rally_steps": snapshot.rally_steps, "fly_hits": snapshot.fly_hits, "fly_misses": snapshot.fly_misses,
                    "brain_latency_ms": brain_latency, "fps": fps,
            ...
            if dashboard:
                if not dashboard.pump(): break
                dashboard.draw(env, decision, fps)
                dashboard.clock.tick(60)
    finally:
        if logger: logger.close()
        if dashboard: dashboard.pg.quit()
    snap = env.snapshot()
    return {
        "controller": controller_name, "seed": seed, "steps": len(actions), "fly_score": snap.fly_score,
        "opponent_score": snap.opponent_score, "mean_brain_latency_ms": float(np.mean(latencies)) if latencies else None,
...
def main() -> None:
    ...
    parser.add_argument("--controller", choices=["fly", "random"], default="fly")
