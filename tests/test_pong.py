...
def test_controller_receives_only_pixels():
    env = PongEnv(seed=2)
    frame = env.render_frame(sensor=True)
    assert isinstance(frame, np.ndarray) and frame.ndim == 2
    ctl = RandomController(2)
    action = ctl.act(frame)
    env.step(action, .02)
...
def test_control_baselines_are_deterministic():
    frame = np.zeros((10, 10), dtype=np.float32)
    assert NeutralController().act(frame) == NEUTRAL
...
def test_headless_baseline_runs_without_brain_data(tmp_path, monkeypatch):
    ...
    for name in ("random", "neutral", "matched-random"):
        result = run_experiment(name, .2, 1, "cpu", False, False)
...
def test_headless_fly_experiment_with_tiny_graph(tmp_path, monkeypatch):
    ...
    ann = pd.DataFrame({
        "bodyId": [1, 2, 3, 4],
        "type": ["LC10a", "LC10a", "DNa02", "DNa02"],
        "superclass": ["visual_projection", "visual_projection", "descending_neuron", "descending_neuron"],
        "somaSide": ["L", "R", "L", "R"],
        "rootSide": ["L", "R", "L", "R"],
    })
